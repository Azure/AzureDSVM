...
Hotspot analysis.
    ...
    0. Given a labelled data set. 
    1. Use k-means to do clustering with n segments.
    2. Build trees/forests to do binary classification to differentiate each of the cluster.
    3. Within each cluster, build a tree/forest to do binary classification on label.
...
```{r}
COUNT <- 6  # Number of VMs to deploy.
...
The R codes for Hot Spot analysis are available as [workerHotSpots.R](https://www.microsoft.com). Besides the main worker script, there are several other scripts where functions used for the analysis reside.
    ...
    * [workerHotSpotsSetup.R](http://) function that sets up the environment.
    * [workerHotSpotsFuncs.R](http://) functions used in training or testing process.
    * [workerHotSpotsTrain.R](http://) functions for training a Hot Spot model for binary classification. 
    * [workerHotSpotsTest.R](http://) functions for testing performance of a classification model based on Hot Spots method.
