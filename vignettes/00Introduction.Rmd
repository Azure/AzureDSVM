...
`AzureDSVM` offers functions that can be conveniently used by R data scientists for operating and using Azure Data Science Virtual Machine (DSVM) elastically and economically, within local R session. The package is useful to programmtically manage DSVMs for a solo or collaborative data science projects so that R-user data scientists do not have to bother interactiving with GUI portal or learning unfamiliar scripting languages for meeting the same desire. 
