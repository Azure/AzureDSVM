...
We can install some useful tools on a fesh server together.

```{r useful tools}
system(paste(ssh, "sudo locale-gen 'en_AU.UTF-8'"))
system(paste(ssh, "sudo apt-get install wajig lsb htop"))
system(paste(ssh, "lsb-release"))
```

It is also recommened that you log on to the server through the secure
shell and manually update the operating system with all packages
updated to their latest versions. You may be asked to accept the
changes. Then reboot the server so that, for example, kernel updates,
take effect.
